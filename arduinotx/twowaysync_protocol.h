...
#define MAX_PKT           27    // radio config length = 25 + rssi + crc status = 27
#define PKT_HEAD          235
...
class TwoWaySyncProtocol: public Protocol {
    public:
        ...
        void init() {
            ...
            uint8_t response_buff[sizeof(WelcomebackPkt)];
            resetModule(0);
            do {
                ...
                CC2500_WriteData((uint8_t*)&hello, sizeof(hello));
            } while(receive(response_buff, 10) == false); // wait until

            WelcomebackPkt* res = (WelcomebackPkt*)response_buff;
            if (res->pkt_type == HELLO_PKT && res->addr == fixed_id) {
              pair();
            } else if (res->pkt_type == WELCOMEBACK_PKT && res->addr == fixed_id) {
        ...
        void transmitAndReceive() {
          ...
          if (begin - lastTransmit > 13) {
            lastTransmit = millis();
            buildDataPacket(packet);
            transmit(hop_channels[curChannel], packet, 18);
            receive(packet, 9);    // TODO: need fine tunning timeout
    ...
    private:
        ...
        void pair() {
            ...
            for (uint8_t i = sizeof(hop_data) - 1; i >=0; --i) {
              transmit(hop_data[i], (uint8_t*)&testPkt, sizeof(testPkt));
              if (!receive(testResPkt, 10)) {
                return;
              } else if (i == 0) {
                WelcomebackPkt* fin = (WelcomebackPkt*) testResPkt;
        ...
        bool receive(uint8_t* buffer, uint16_t timeout) {
        ...
        void buildDataPacket(uint8_t* sbus_data) {
          sbus_data[0] = PKT_HEAD;
          sbus_data[1] = DATA_PKT;
        ...
        void resetModule(uint8_t bind) {
            ...
            CC2500_WriteReg(CC2500_11_MDMCFG3, 0x61); //DRATE_M = 0x61 Bitrate = 70023 bps
